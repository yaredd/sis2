class Period < ActiveRecord::Base
  ...
  def self.current_semester
  	where("endDate >= ? and startDate <= ?", Date.today, Date.today).where("name like 'Sem%'").first.name.titleize
  end
  
  def  self.current_period
    where("endDate >= ? and startDate <= ?", Date.today, Date.today).where("name like 'Q%'").first.name
  end
